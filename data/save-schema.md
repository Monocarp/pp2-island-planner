...
- **Rickyard / Paddock (save analysis):** Game saves use entity id **`Silo`** for rickyards. **5×5** footprint (Chebyshev distance ≤ 2 from silo/paddock anchor). A collector **cannot share the anchor cell** with the silo/paddock (each building has its own anchor); boosts apply only when the collector anchor is **inside** the footprint but **not** identical to the boost source anchor. Then **×2** output (after tile utilization). **Rickyard:** pigs, cattle, sheep, horses only. **Paddock:** entities with a **`harvester`** component. **No stacking** (at most ×2 from this rule).
