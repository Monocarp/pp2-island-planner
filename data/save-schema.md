...
- **Rickyard / Paddock (save analysis):** Saves use **`Silo`** for rickyards. Silo and Paddock are **boost sources only** (not production rows). **5×5** area: **Chebyshev distance ≤ 2** from the silo/paddock **anchor** — i.e. the axis-aligned square with corners **`(cx−2, cy−2)`** through **`(cx+2, cy+2)`** (same as “radius 2” in L∞ norm). Parser fields: **`insideSiloFootprint`** = eligible livestock anchor in that area; **`siloBoosted`** = that **and** **full** tile utilization (then output gets ×2 from rickyard). Partial grass scales the base rate down and **does not** get rickyard ×2 in the parser (UI may still highlight “in rickyard” separately from the ×2 production line). **Paddock ×2:** entities with a **`harvester`** component whose anchor is in the paddock footprint, and **not** a rickyard-eligible building in a silo footprint (no stacking). Valid saves do not place two building anchors on the same cell.
...
- **Spatial inputs (grass, deposits, `water_tile`, etc.):** **`grass`** and **`water_tile`** use **per-cell 1/n** splitting among all overlapping livestock / water gatherer footprints. Other tile resources use **independent** per-building cell counts (same-type farms on one field each count full cells). **Rickyard ×2** for pigs/cattle/sheep/horses requires **full** spatial utilization in the save-analysis model.
