...
- **Terrain budget** (`computeTerrainBudget` in `planner.js`): counts unoccupied cells valid for `straight_river`, `ocean_adjacent`, `river_adjacent`, `in_water_coastal`, and **free land** (grass/forest, no building).
- **Location constraints**: sums fractional building counts from the chain that share each `LOCATION_REQUIREMENTS` type (e.g. several buildings competing for river anchors); capacity for that type is `budget / sumPerHouse`.
- **Land constraint**: `1 + productionChainBuildingCount + count(service resources in house consumePerMinute) + 1` (warehouse estimate) anchors per house vs free land cells.
