...
| `validation.js` | Island stats (including **Island capacity** and **Population supply**), validation (warehouse/service coverage, tile resources, overlap) |
| `planner.js` | Production chain solver, `NATURAL_DEPOSIT_IDS`, demand calculator, `resolveProductionChainWithImports`, `computeTerrainBudget`, `computeIslandCapacity`, `autoPopulateFromResolvedChain`, auto-populate |
...
| `save-import.js` | PP2 game save (`.dat`) importer: `parsePP2SaveFile`, `applyPP2SaveImport`, `showSaveImportModal`, `triggerSaveFileImport`, `initSaveImport`; **Import save** header button (visible in multi-island mode) |
...
```
data.js → buildings.js → island.js → ships.js → renderer.js → ui.js → validation.js → planner.js → multi-planner.js → save-import.js → saveload.js
```

Each file depends on globals from previous files. There is no module system.

### Island Stats — Island capacity (estimated max houses)

**Island capacity** appears in **Island Stats** for every grid (no houses required). For each **visible** population tier (`getVisiblePopBuildings`), if that house type is **unlocked**, the tool runs `resolveProductionChain` on the goods demand of **one** house of that type, then estimates how many such houses the **current terrain** could support before the tightest constraint binds.

- **Terrain budget** (`computeTerrainBudget(island?)` in `planner.js`): counts unoccupied cells on the given grid (default `state.island`) valid for `straight_river`, `ocean_adjacent`, `river_adjacent`, `in_water_coastal`, and **free land** (grass/forest, no building). River “straight” detection uses the same rules as water-wheel placement for that grid.
- **Optional military overhead** (`computeIslandCapacity({ island, militaryEntries })`): resolves `getMilitaryDemandFromEntries` + `inferMilitiaHuts` + hut sub-chain (`mergeMilitaryChainWithMilitiaHuts`), then subtracts location usage and land anchors (`landAnchorEstimateFromChain`) from the budget before dividing by per-house usage. Island Stats calls `computeIslandCapacity()` with no options (no military).
- **Location constraints**: sums fractional building counts from the chain that share each `LOCATION_REQUIREMENTS` type (e.g. several buildings competing for river anchors); capacity for that type is `(budget − militaryUsage) / sumPerHouse`.
- **Land constraint**: `1 + productionChainBuildingCount + count(service resources in house consumePerMinute) + 1` (warehouse estimate) anchors per house vs `(land − militaryLandAnchors)` free land cells.
- **Per tier in isolation**: if you only built Colonist Houses, not a mixed island. Mixed populations and **tile/deposit** limits (beyond location rules) are future refinements.
- **Imports**: assumes everything is produced locally (same as planner default).

`validation.js` calls `computeIslandCapacity()` only after `planner.js` has loaded (stats refresh runs at runtime).
...
- **Switch island slot** dropdown + optional **Island name** + **Delete this island** (removes the **currently selected** slot from `projectSlots`; confirms if the slot has grid content; at least one island must remain; archetype counts are derived from remaining slots). **Island counts** (header) only trims slots from the **end** of the temperate block and/or tropical block—use **Delete this island** to drop a specific middle slot without changing the other block.
- Temperate/Tropical **type bar is hidden** in project mode; archetype comes from the slot.
- **`setActiveSlot`**, **`saveProjectToStorage`**, **`loadProjectFromStorage`**, **`commitActiveSlotFromState`**, **`deleteProjectSlotAt`** (saveload.js); **`buildSlotSelectorUI`**, **`syncMultiIslandUI`** (ui.js). `beforeunload` commits the active grid into the slot.
...
- **Plan islands** (header): the modal includes **Military (planning only)** — unit + rate/hr entries copied from **`state.militaryEntries`** on open; they adjust **per-slot Island capacity** tables (`computeIslandCapacityForProjectSlot` + each slot’s saved `island` grid + `withProjectSlotContext` fertilities). Values are **not** synced back to the main planner. **Analyze** still uses **population counts only** (no modal military in `analyzeMultiIslandPlan` yet). Producer **⇄** overrides in the modal refresh both results and the capacity panel. Total population counts → **`analyzeMultiIslandPlan`** enumerates home island + which intermediate good to ship (e.g. wheat vs flour vs bread, cider from apple islands) using **`state.projectShipCounts`** throughput; **`executeMultiIslandPlan`** runs **`autoPopulateFromResolvedChain`** on source slot(s) then home. Requires ≥2 slots with grids. Feasibility treats **`canSlotProduceResourceFully`** as false when **`resolveProductionChain`** yields no buildings and no tile needs for that demand (e.g. no local cider without apple fertility)—so those goods enter cross-island options instead of being skipped. **Fertility sync:** checkbox changes update **`projectSlots[activeSlotIndex].activeFertilities`** (multi-island); **`commitActiveFertilitiesToActiveSlot`** when opening the modal / Analyze / Execute; **`withProjectSlotContext`** uses **live `state.activeFertilities`** for the **active** slot index and **`effectiveSlotFertilitySet(slot)`** for others. **`effectiveSlotFertilitySet`** treats **`[]`** as no tile fertilities (not “all defaults”).
- **No valid distribution options:** when **Analyze** finds no plan, **`analyzeMultiIslandPlan`** returns **`diagnostics.messages`** built by **`buildMultiIslandEmptyOptionsDiagnostics`**: per-good **no supplier** lines with **`getSlotProductionFeasibility`** reasons merged across slots, plus deduped **NO_SHIP_CHAIN** lines (supplier slot label, empty ship trace). The modal still shows **fleet capacity** on that screen.
...
- **Import save (header button):** visible in multi-island mode. Reads a PP2 `.dat` game save (JSON) and maps `IslandManager.islands` to planner slots via `parsePP2SaveFile` (`save-import.js`) + `applyPP2SaveImport`. Grid `Type 2` = water; `Type 3 + Height` = grass; `Type 3 + DepositType` = deposit (`SAVE_DEPOSIT_TYPE_INT`). `GameEntities` terrain IDs (`Forest`) set `cell.terrain`; field entities (`AppleTrees`, `WheatField`, etc.) set `cell.deposit`; buildings are mapped via direct match or `SAVE_BUILDING_ID_REMAP`; infrastructure (`Kontor1`, garrisons, portals) is skipped. Fertilities are inferred from deposit tiles. Region 1 = temperate; 2/4 = tropical. **`ShipManager.Ships`:** Type 2/3/4 → `caravel` / `hulk` / `pinnace` into `state.projectShipCounts` (`applyProjectShipCountsFromPayload`). **House entities** (`House0Pioneers` … `House6Paragons`) increment per-island `importedPopCounts` and pre-fill the main planner population inputs for the **first** slot after import. A confirmation modal shows fleet, per-island houses, and entity summary before overwriting the project. Rivers and coastal tiles must be painted manually afterward.
