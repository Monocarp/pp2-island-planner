...
- **Import save (header button):** visible in multi-island mode. Reads a PP2 \.dat\ game save (JSON) and maps \IslandManager.islands\ to planner slots via \parsePP2SaveFile\ (\save-import.js\) + \pplyPP2SaveImport\. Grid \Type 2\ = water; \Type 3 + Height\ = grass; \Type 3 + DepositType\ = deposit (\SAVE_DEPOSIT_TYPE_INT\). \GameEntities\ terrain IDs (\Forest\) set \cell.terrain\; field entities (\AppleTrees\, \WheatField\, etc.) set \cell.deposit\; buildings are mapped via direct match or \SAVE_BUILDING_ID_REMAP\; infrastructure (\Kontor1\, garrisons, portals) is skipped. Fertilities are inferred from deposit tiles. Region 1 = temperate; 2/4 = tropical. A confirmation modal shows entity summary before overwriting the project. Rivers and coastal tiles must be painted manually afterward.
