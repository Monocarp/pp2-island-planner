...
- **Plan islands** (header): total population counts → **`analyzeMultiIslandPlan`** enumerates home island + which intermediate good to ship (e.g. wheat vs flour vs bread, cider from apple islands) using **`state.projectShipCounts`** throughput; **`executeMultiIslandPlan`** runs **`autoPopulateFromResolvedChain`** on source slot(s) then home. Requires ≥2 slots with grids. Feasibility treats **`canSlotProduceResourceFully`** as false when **`resolveProductionChain`** yields no buildings and no tile needs for that demand (e.g. no local cider without apple fertility)—so those goods enter cross-island options instead of being skipped. **Analyze / Execute call `commitActiveSlotFromState` first** so **`projectSlots[].activeFertilities`** matches the fertility checkboxes on the **currently visible** island (checkboxes only update `state.activeFertilities` until slot switch or commit).
